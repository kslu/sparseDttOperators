...
int main(int argc, char *argv[]) {
  ...
  const double *pgf_coeffs_ptr[PGFDEG] = {
      diff16x16_pgf1_coeffs, diff16x16_pgf2_coeffs, diff16x16_pgf3_coeffs,
      diff16x16_pgf4_coeffs, diff16x16_pgf5_coeffs, diff16x16_pgf6_coeffs,
      diff16x16_pgf7_coeffs, diff16x16_pgf8_coeffs, diff16x16_pgf9_coeffs,
      diff16x16_pgf10_coeffs};
  const double *cpgf_coeffs_ptr[PGFDEG] = {
      diff16x16_cpgf1_coeffs, diff16x16_cpgf2_coeffs, diff16x16_cpgf3_coeffs,
      diff16x16_cpgf4_coeffs, diff16x16_cpgf5_coeffs, diff16x16_cpgf6_coeffs,
      diff16x16_cpgf7_coeffs, diff16x16_cpgf8_coeffs, diff16x16_cpgf9_coeffs,
      diff16x16_cpgf10_coeffs};
  const double *mpgf_coeffs_ptr[MPGFDEG * MAXM] = {
      diff16x16_mpgfl1m1_coeffs, diff16x16_mpgfl1m2_coeffs, diff16x16_mpgfl1m3_coeffs,
      diff16x16_mpgfl1m4_coeffs, diff16x16_mpgfl1m5_coeffs, diff16x16_mpgfl1m6_coeffs,
      diff16x16_mpgfl1m7_coeffs, diff16x16_mpgfl1m8_coeffs, diff16x16_mpgfl2m1_coeffs,
      diff16x16_mpgfl2m2_coeffs, diff16x16_mpgfl2m3_coeffs, diff16x16_mpgfl2m4_coeffs,
      diff16x16_mpgfl2m5_coeffs, diff16x16_mpgfl2m6_coeffs, diff16x16_mpgfl2m7_coeffs,
      diff16x16_mpgfl2m8_coeffs};
  const int *mpgf_powers_ptr[MPGFDEG * MAXM] = {
      diff16x16_mpgfl1m1_powers, diff16x16_mpgfl1m2_powers, diff16x16_mpgfl1m3_powers,
      diff16x16_mpgfl1m4_powers, diff16x16_mpgfl1m5_powers, diff16x16_mpgfl1m6_powers,
      diff16x16_mpgfl1m7_powers, diff16x16_mpgfl1m8_powers, diff16x16_mpgfl2m1_powers,
      diff16x16_mpgfl2m2_powers, diff16x16_mpgfl2m3_powers, diff16x16_mpgfl2m4_powers,
      diff16x16_mpgfl2m5_powers, diff16x16_mpgfl2m6_powers, diff16x16_mpgfl2m7_powers,
      diff16x16_mpgfl2m8_powers};
  const double *arma_b_ptr[ABDEGS] = {diff16x16_armad1_b, diff16x16_armad2_b,
                               diff16x16_armad3_b};
  const double *arma_a_ptr[ABDEGS] = {diff16x16_armad1_a, diff16x16_armad2_a,
                               diff16x16_armad3_a};
  ...
  for (int b = 0; b < n_batches; b++) {
    ...
    t_temp = clock();
    for (int i = 0; i < cur_batch_size; i++)
      exact_filter_16x16(buffer_in[i], buffer_out_exact[i], h16x16_diff);
    t_exact += clock() - t_temp;

    // matrix multiplication
    t_temp = clock();
    for (int i = 0; i < cur_batch_size; i++)
      mat_times_vec(buffer_in[i], buffer_out_mat[i], diff16x16, LEN);
    t_mat += clock() - t_temp;
    for (int i = 0; i < cur_batch_size; i++) {
      for (int j = 0; j < LEN; j++) {
