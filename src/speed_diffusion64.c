...
int main(int argc, char *argv[]) {
  ...
  const double *pgf_coeffs_ptr[PGFDEG] = {
      diff64_pgf1_coeffs, diff64_pgf2_coeffs, diff64_pgf3_coeffs,
      diff64_pgf4_coeffs, diff64_pgf5_coeffs, diff64_pgf6_coeffs,
      diff64_pgf7_coeffs, diff64_pgf8_coeffs, diff64_pgf9_coeffs,
      diff64_pgf10_coeffs};
  const double *cpgf_coeffs_ptr[PGFDEG] = {
      diff64_cpgf1_coeffs, diff64_cpgf2_coeffs, diff64_cpgf3_coeffs,
      diff64_cpgf4_coeffs, diff64_cpgf5_coeffs, diff64_cpgf6_coeffs,
      diff64_cpgf7_coeffs, diff64_cpgf8_coeffs, diff64_cpgf9_coeffs,
      diff64_cpgf10_coeffs};
  const double *mpgf_coeffs_ptr[16] = {
      diff64_mpgfl1m1_coeffs, diff64_mpgfl1m2_coeffs, diff64_mpgfl1m3_coeffs,
      diff64_mpgfl1m4_coeffs, diff64_mpgfl1m5_coeffs, diff64_mpgfl1m6_coeffs,
      diff64_mpgfl1m7_coeffs, diff64_mpgfl1m8_coeffs, diff64_mpgfl2m1_coeffs,
      diff64_mpgfl2m2_coeffs, diff64_mpgfl2m3_coeffs, diff64_mpgfl2m4_coeffs,
      diff64_mpgfl2m5_coeffs, diff64_mpgfl2m6_coeffs, diff64_mpgfl2m7_coeffs,
      diff64_mpgfl2m8_coeffs};
  const int *mpgf_powers_ptr[16] = {
      diff64_mpgfl1m1_powers, diff64_mpgfl1m2_powers, diff64_mpgfl1m3_powers,
      diff64_mpgfl1m4_powers, diff64_mpgfl1m5_powers, diff64_mpgfl1m6_powers,
      diff64_mpgfl1m7_powers, diff64_mpgfl1m8_powers, diff64_mpgfl2m1_powers,
      diff64_mpgfl2m2_powers, diff64_mpgfl2m3_powers, diff64_mpgfl2m4_powers,
      diff64_mpgfl2m5_powers, diff64_mpgfl2m6_powers, diff64_mpgfl2m7_powers,
      diff64_mpgfl2m8_powers};
  const double *arma_b_ptr[ABDEGS] = {diff64_armad1_b, diff64_armad2_b,
                               diff64_armad3_b};
  const double *arma_a_ptr[ABDEGS] = {diff64_armad1_a, diff64_armad2_a,
                               diff64_armad3_a};
  ...
  for (int b = 0; b < n_batches; b++) {
    ...
    t_temp = clock();
    for (int i = 0; i < cur_batch_size; i++)
      exact_filter_64(buffer_in[i], buffer_out_exact[i], h64_diff);
    t_exact += clock() - t_temp;

    // matrix multiplication
    t_temp = clock();
    for (int i = 0; i < cur_batch_size; i++)
      mat_times_vec(buffer_in[i], buffer_out_mat[i], diff64, LEN);
    t_mat += clock() - t_temp;
    for (int i = 0; i < cur_batch_size; i++) {
      for (int j = 0; j < LEN; j++) {
    ...
    for (int ord = 1; ord <= PGFDEG; ord++) {
      ...
      for (int i = 0; i < cur_batch_size; i++)
        chebyshev_gf(buffer_in[i], buffer_out_cpgf[ord - 1][i], LEN, ord,
                     cpgf_coeffs_ptr[ord - 1], NE_LD64, MEV_LD64, Ld64_a,
                     Ld64_w);
      ...
      for (int it = 1; it <= NITS; it++) {
        ...
        for (int i = 0; i < cur_batch_size; i++)
          armagf_cg(buffer_in[i], buffer_out_arma[ind][i], LEN, it, kbs[d],
                    arma_b_ptr[d], kas[d], arma_a_ptr[d], NE_LD64, Ld64_a,
                    Ld64_w);
