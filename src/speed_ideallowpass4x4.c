...
int main(int argc, char *argv[]) {
  ...
  for (int b = 0; b < n_batches; b++) {
    ...
    t_temp = clock();
    for (int i = 0; i < cur_batch_size; i++)
      mat_times_vec(buffer_in[i], buffer_out_exact[i], lp4x4, LEN);
    t_mat += clock() - t_temp;

    // FIR graph filter
    const double *fir_coeffs_ptr[10] = {lp4x4_fir1_coeffs, lp4x4_fir2_coeffs,
                                        lp4x4_fir3_coeffs, lp4x4_fir4_coeffs,
                                        lp4x4_fir5_coeffs, lp4x4_fir6_coeffs,
                                        lp4x4_fir7_coeffs, lp4x4_fir8_coeffs,
                                        lp4x4_fir9_coeffs, lp4x4_fir10_coeffs};
